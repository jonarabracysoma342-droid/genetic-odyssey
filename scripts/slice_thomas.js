...
const inputPath = 'C:/Users/ADVAN/.gemini/antigravity-ide/brain/4266137e-7a8a-4970-86bc-f00ce079a0ad/.user_uploaded/media_1789122984751.jpg';
const outDir = path.resolve('public/assets/rpg/npc/monk');

if (!fs.existsSync(outDir)) {
  fs.mkdirSync(outDir, { recursive: true });
}

async function sliceThomas() {
  ...
  const slices = [
    ...
    // Row 0 right: Walk Side (4 frames)
    { name: 'right_0', x: 4 * 128, y: 0, w: 128, h: 177 },
    { name: 'right_1', x: 5 * 128, y: 0, w: 128, h: 177 },
    { name: 'right_2', x: 6 * 128, y: 0, w: 128, h: 177 },
    { name: 'right_3', x: 7 * 128, y: 0, w: 128, h: 177 },

    // Row 1 right: Walk Up (4 frames)
    ...
    // Row 2 (Bottom): Special & Idle
    { name: 'idle_0', x: 0 * 256, y: 357, w: 256, h: 202 },
    { name: 'idle_1', x: 1 * 256, y: 357, w: 256, h: 202 },
    { name: 'dig_0',  x: 2 * 256, y: 357, w: 256, h: 202 },
    { name: 'sweat_0',x: 3 * 256, y: 357, w: 256, h: 202 },
  ];
  ...
  for (const s of slices) {
    ...
    const isBg = (x, y) => {
      ...
      // Checkerboard greys (low saturation)
      const maxDiff = Math.max(Math.abs(r - g), Math.abs(g - b), Math.abs(r - b));
      if (maxDiff <= 8 && r >= 90 && r <= 220) return true;
    ...
    for (let y = 0; y < s.h; y++) {
      for (let x = 0; x < s.w; x++) {
        ...
        if ((maxDiff <= 10 && r >= 85 && r <= 220 && solidNeighbors <= 3) || solidNeighbors <= 1) {
          cellBuf[idx + 3] = 0;
        }
      }
    }
...
sliceThomas().catch(console.error);
