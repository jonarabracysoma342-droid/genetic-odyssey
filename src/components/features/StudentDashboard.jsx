...
export const StudentDashboard = () => {
  ...
  // Material details & Q&A comments modal state
  ...
  const handleSubmitMaterialComment = async (e) => {
    e.preventDefault();
    if (!commentInput.trim() || !activeMaterialModal || !groupId) return;
    ...
    const newComment = {
      ...
      authorName: userName || currentUser?.displayName || 'Siswa',
      authorRole: 'siswa',
    ...
    try {
      const updatedMaterials = (studentGroupInfo.materials || []).map(mat => {
        if (mat.id === activeMaterialModal.id) {
          const currentComments = mat.comments || [];
          return {
            ...mat,
      ...
      setActiveMaterialModal(prev => ({
        ...prev,
        comments: [...(prev.comments || []), newComment]
      }));
      setStudentGroupInfo(prev => ({
        ...prev,
        materials: updatedMaterials
      }));
    } catch (err) {
      sound.playWrong();
      console.error("Error submitting material comment:", err);
      showAlert("Gagal mengirim pertanyaan. Silakan coba kembali.");
    } finally {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {/* DETAIL MATERI & DISKUSI TANYA JAWAB MODAL */}
      {activeMaterialModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in text-left">
          <div className="bg-white border-[3px] border-slate-800 rounded-3xl w-full max-w-md overflow-hidden shadow-[8px_8px_0px_#1e293b] flex flex-col max-h-[88vh] animate-scale-up">
            ...
            <div className="p-4 bg-indigo-50 border-b-[3px] border-slate-800 flex justify-between items-center flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-[11px] font-black text-slate-855 uppercase leading-none font-sans">Materi Belajar & Diskusi</h4>
            ...
            <div className="p-4 overflow-y-auto space-y-4 flex-1 text-left">
              ...
              {/* Material Text */}
              <div className="space-y-2 border-b-2 border-slate-100 pb-3">
                <h3 className="text-sm font-black text-slate-900 leading-tight">{activeMaterialModal.title}</h3>
                <p className="text-[10px] font-medium text-slate-700 leading-relaxed whitespace-pre-wrap">{activeMaterialModal.content}</p>
                
                {activeMaterialModal.url && (
                  <div className="p-2.5 border border-indigo-200 bg-indigo-50/40 rounded-xl space-y-1">
                    <span className="text-[7.5px] font-black text-indigo-700 uppercase tracking-wide block">Tautan Belajar Eksternal</span>
                    <a 
              ...
              {/* Discussion / Q&A Thread Section */}
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-[10px] font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                    <MessageSquare className="w-3.5 h-3.5 text-indigo-600" /> Tanya Jawab Materi ({(activeMaterialModal.comments || []).length})
                  </h4>
                  <span className="text-[8px] font-bold text-slate-400">Tanya langsung ke guru</span>
                </div>

                {/* Comments List */}
                {(!activeMaterialModal.comments || activeMaterialModal.comments.length === 0) ? (
                  <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-center">
                    <p className="text-[9.5px] font-medium text-slate-500">
                      Belum ada pertanyaan. Jika ada konsep yang belum dipahami, tanyakan di bawah ini!
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2.5">
                    {activeMaterialModal.comments.map((comm) => (
                      <div key={comm.id} className="p-3 bg-slate-50 border-2 border-slate-200 rounded-2xl space-y-1.5 shadow-2xs">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className={`px-1.5 py-0.2 rounded text-[7px] font-black uppercase ${
                              comm.authorRole === 'guru' ? 'bg-indigo-100 text-indigo-800' : 'bg-emerald-100 text-emerald-800'
                            }`}>
                        ...
                        <p className="text-[9.5px] font-medium text-slate-700 leading-relaxed">{comm.text}</p>
                        
                        {/* Nested Replies */}
                        {comm.replies && comm.replies.length > 0 && (
                          <div className="pl-2.5 border-l-2 border-indigo-400 space-y-1.5 pt-1.5 mt-1.5">
                            {comm.replies.map(rep => (
                              <div key={rep.id} className="p-2 bg-indigo-50/90 border border-indigo-200 rounded-xl space-y-0.5">
                                <div className="flex items-center gap-1.5">
                                  <span className="text-[9px] font-black text-indigo-950">{rep.authorName}</span>
                                  <span className="px-1 py-0.2 rounded text-[6.5px] font-black bg-indigo-200 text-indigo-900 uppercase">
                                    {rep.authorRole === 'guru' ? '👨‍🏫 Guru' : 'Siswa'}
                                  </span>
                                </div>
                                <p className="text-[9px] font-medium text-slate-800 leading-relaxed">{rep.text}</p>
                              </div>
                            ))}
                          </div>
                        )}
                ...
                {/* Question Input Form */}
                <form onSubmit={handleSubmitMaterialComment} className="pt-2 border-t border-slate-200 space-y-2">
                  <span className="text-[8px] font-black text-indigo-700 uppercase tracking-wider block">
                    Belum Paham? Tanyakan ke Guru:
                  </span>
                  <div className="flex gap-1.5">
                    <input
                      ...
                      className="flex-1 px-3 py-2 text-[9.5px] font-bold bg-white border-2 border-slate-800 rounded-xl placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-3xs"
                    />
                    <button
                      type="submit"
                      disabled={submittingComment || !commentInput.trim()}
                      className="px-3.5 py-2 bg-indigo-650 hover:bg-indigo-700 text-white font-black text-[9px] border-2 border-slate-800 rounded-xl shadow-3xs flex items-center gap-1 cursor-pointer disabled:opacity-50 active:translate-y-0.5 active:shadow-none transition"
                    >
                      <Send className="w-3.5 h-3.5" />
                      <span>{submittingComment ? '...' : 'KIRIM'}</span>
                    </button>
                  </div>
                </form>
            ...
            <div className="p-3 bg-slate-50 border-t-2 border-slate-800 flex justify-end">
              <button
                onClick={() => setActiveMaterialModal(null)}
                className="px-4 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-800 font-black text-[9px] border-2 border-slate-800 rounded-xl shadow-[2px_2px_0px_#1e293b] active:translate-y-0.5 active:shadow-none transition-all cursor-pointer"
              >
                TUTUP
              </button>
            </div>
